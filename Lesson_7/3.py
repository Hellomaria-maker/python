...
class Cells:
    ...
    def __add__(self, other):
        if isinstance(other.count, int) and isinstance(other, Cells):
            return Cells(self.count + other.count)
        else:
            return "Ошибка сложения!"

    def __sub__(self, other):
        if isinstance(other.count, int) and isinstance(other, Cells) and self.count - other.count > 0:
            return Cells(self.count - other.count)
        else:
    ...
    def __str__(self):
        return f"Количесво ячеек после выполнения операции: {self.count}"

    def make_order(self, raw):
        while self.count >= raw:
            print('*' * raw, end='\n')
            self.count -= raw
        print('*' * self.count)
